...
export const PROJECTS_COMMERCIAL: Project[] = [
  ...
  },
  {
    ...
    slug: "it-studio",
    category: "commercial",
    name: "Студия IT-разработки",
    tag: "Кастомные продукты и AI",
    short: "Веб, мобильные и AI-решения под боли бизнеса.",
    long:
      "Собираем команду под задачу. От прототипа до полной разработки. Особый фокус на продуктах с нейросетями, которые закрывают конкретную операционку. Внутри направления живёт Autonomy AI: Telegram-канал о нейросетях и курс по нейросетям для предпринимателей и команд.",
  ...
  },
  {
    ...
    tag: "HR-направление",
    short: "От бизнес-ассистента до C-level.",
    long:
      "Подбираем команды для предпринимателей: бизнес-ассистенты, операционные менеджеры, маркетологи, руководители уровня C-level. Работаем с проверенной воронкой кандидатов и сообществом молодёжи титульных вузов России. Это даёт скорость, которую сложно получить через внешних рекрутеров.",
  ...
  },
  {
    ...
    long:
      "Два направления. Старшеклассникам помогаю поступить в МФТИ и другие топовые вузы через олимпиады и ЕГЭ. Студентам помогаю найти направление и запустить первый проект. Куратор программы Олеся Сычева.",
  ...
  },
  {
    ...
    long:
      "Первый офлайн-бизнес. Сеть кофейных автоматов в Петрозаводске. Опыт операционки, который держит на земле и учит управлять физическим продуктом, а не только цифровым.",
...
export const PROJECTS_SOCIAL: Project[] = [
  {
    ...
    short: "Способ сказать спасибо преподавателю и поддержать его подпиской.",
    long:
      "Сейчас в разработке. Идея простая: дать выпускникам способ напрямую поддерживать тех, кто их учил. Чтобы преподаватель не выживал на 30 000 рублей, а получал благодарность от тех, кому изменил траекторию. Команда проекта: Антон Орешкин, Виктор Кузнецов, Алексей Штерн, Виктория Яськова.",
  ...
  },
  {
    ...
    long:
      "Сообщество студентов титульных вузов России. Встречаемся, проводим курс по нейросетям для студентов, помогаем ребятам устраиваться к предпринимателям. АССИСТ+ это не агентство, это сообщество, где люди находят друг друга.",
  ...
  },
  {
    ...
    category: "social",
    name: "Форум-группа",
    tag: "Заряженная молодёжь",
    short: "Студенты, у которых амбиции подкреплены поступками.",
    long:
      "Сообщество, где мы собираемся группой и делимся прогрессом, ошибками, опытом. Это формат, который помогает каждому участнику расти быстрее, потому что рядом такие же определившиеся ребята. Открыто для студентов, которые хотят больше, чем учёба.",
...
export const FOOTER_PROJECTS = [
  ...
  { name: "Студия IT-разработки", href: "/it-studio" },
